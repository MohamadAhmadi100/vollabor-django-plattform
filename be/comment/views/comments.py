...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if author_email:
            email = EmailMessage(
                        "A new comment", 
                        "A new comment posted on <{}> that you are the author of\n{}{}".format(article, current_site, reverse('forum:detail-title' , kwargs={'pk': article.pk})), 
                        to=[author_email]
            )
            email.send()
        ...
        if parent_email:
            email = EmailMessage(
                        "A comment reply ", 
                        "Someone has sent a reply to your comment on the topic <{}> Click the link below for more details: \n{}{}".format(article, current_site, reverse('forum:detail-title' , kwargs={'pk': article.pk})), 
                        to=[parent_email]
            )
            email.send()
